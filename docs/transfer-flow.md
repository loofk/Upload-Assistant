...
`resume_state.materials.recovery_hints` 也会保留具体的描述缺口，例如 `description.ptgen_description`、`description.screenshot_bbcode` 和 `description.screenshot_coverage`；其中缺本地截图会触发 `--generate-screenshots`，缺图床覆盖会触发 `--upload-screenshots`，仅缺描述 BBCode 时会优先重新生成目标包描述。
