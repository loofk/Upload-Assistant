...
def build_parser() -> argparse.ArgumentParser:
    ...
    daily_schedule.add_argument("--notification-webhook-url", help="Optional HTTP webhook URL for pushing the daily candidate notification payload.")
    daily_schedule.add_argument("--notification-webhook-timeout", type=float, default=10.0, help="Webhook POST timeout in seconds. Defaults to 10.")
    ...
    daily_scheduler.add_argument("--notification-webhook-url", help="Optional HTTP webhook URL for pushing the daily candidate notification payload after each run.")
    daily_scheduler.add_argument("--notification-webhook-timeout", type=float, default=10.0, help="Webhook POST timeout in seconds. Defaults to 10.")
...
def _material_recovery_missing_priority(missing: str) -> tuple[int, str]:
    normalized = _target_preparation_missing_key(missing)
    if normalized.startswith("metadata.") or normalized.startswith("description.external_ids") or normalized.startswith("payload."):
        return (0, normalized)
    if normalized in {"assets.mediainfo_or_bdinfo", "assets.bdinfo_for_disc", "description.mediainfo_or_bdinfo"}:
        return (1, normalized)
    if normalized == "assets.screenshots":
        return (2, normalized)
    if normalized in {"assets.image_host_uploads", "description.screenshot_coverage"}:
...
def _summary_target_material_metadata_section(target_materials: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "tmdb_type": metadata.get("tmdb_type"),
...
def _summary_check_result(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    first_runnable_command = _preferred_first_runnable_candidate_command(candidate_commands)
...
def _summary_check_readiness_summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if daily_candidate_targets:
        ...
        readiness["daily_candidate_safe_count"] = daily_candidate_targets.get("safe_count")
    approval_queue = payload.get("approval_queue") if isinstance(payload.get("approval_queue"), dict) else {}
    if approval_queue:
        readiness["daily_candidate_approval_queue"] = approval_queue
        readiness["daily_candidate_approval_ready"] = approval_queue.get("ready")
        readiness["daily_candidate_approval_safe_count"] = approval_queue.get("safe_count")
    return readiness
...
def _doctor_summary_check(payload: dict[str, Any], summary_file: str) -> dict[str, Any]:
    ...
    _extend_unique_string(artifact_status["missing_artifacts"], missing_audit)
    blockers = [*blockers, *[f"missing audit artifact: {name}" for name in missing_audit]]
    check_status = "ok" if ready and live_safe and not blockers else "blocked"
    readiness_summary = _summary_check_readiness_summary({
        "status": check_status,
        "summary_file": summary_file,
        "ready": ready,
...
def _daily_schedule_summary_check(payload: dict[str, Any], summary_file: str) -> dict[str, Any]:
    ...
    notification_payload = payload.get("notification_payload") if isinstance(payload.get("notification_payload"), dict) else schedule_digest.get("notification_payload") if isinstance(schedule_digest.get("notification_payload"), dict) else {}
    delivery_handoff = payload.get("delivery_handoff") if isinstance(payload.get("delivery_handoff"), dict) else {}
    approval_queue = _daily_schedule_approval_queue_summary(schedule_digest, notification_payload, delivery_handoff)
    top_safe_candidates = approval_queue.get("top_safe_candidates") if isinstance(approval_queue.get("top_safe_candidates"), list) else []
    ...
    delivery_audit = payload.get("delivery_audit") if isinstance(payload.get("delivery_audit"), dict) else {}
    daily_candidate_targets = _daily_candidate_target_audit(schedule_digest, notification_payload)
    ...
    safe_candidate_count = int(approval_queue.get("safe_count") or len(top_safe_candidates) or 0)
    ...
    if approval_ready:
        next_actions.append("Review approval_queue.items, keep confirm_upload=true plus save_path or path, then submit an approved request_template to submit_daily_candidate_job.")
    elif can_submit_any:
    ...
    return _summary_check_result(
        {
            ...
            "top_safe_candidates": top_safe_candidates,
            "recommended_approval_request": top_safe_candidates[0].get("request_template") if top_safe_candidates and isinstance(top_safe_candidates[0], dict) else None,
...
def _daily_schedule_approval_queue_summary(schedule_digest: dict[str, Any], notification_payload: dict[str, Any], delivery_handoff: dict[str, Any]) -> dict[str, Any]:
    for container in (schedule_digest, notification_payload, delivery_handoff):
        queue = container.get("approval_queue") if isinstance(container.get("approval_queue"), dict) else {}
        if queue:
            return queue
    return {
        ...
        "items": [],
        "top_safe_candidates": [],
        "blockers": ["approval_queue is missing from daily schedule summary."],
        "next_actions": ["Rerun daily-schedule with a current ptcli build to produce approval_queue."],
    }
...
def _daily_candidate_target_audit(schedule_digest: dict[str, Any], notification_payload: Any = None) -> dict[str, Any]:
    ...
    safe_count = _int_or_zero(counts.get("safe_candidates") or schedule_approval_queue.get("safe_count"))
...
def _summary_material_recovery_completion_command_entry(payload: dict[str, Any]) -> dict[str, Any] | None:
    if payload.get("next_command_source") == "material_recovery_completion" and payload.get("next_command"):
        argv = _argv_list(payload.get("next_command_argv")) or _summary_next_command_raw_argv(str(payload.get("next_command")))
        if argv:
            return {
                "stage": payload.get("next_stage") or "resume-target-package",
...
def _preferred_first_runnable_candidate_command(candidate_commands: list[Any]) -> dict[str, Any]:
    for command in candidate_commands:
        if isinstance(command, dict) and command.get("source") == "material_recovery_completion" and command.get("run_allowed") is True:
            return command
    return _first_runnable_candidate_command(candidate_commands)
...
def _material_generation_artifacts(stages: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if isinstance(metadata_stage, dict):
        ...
        artifacts["metadata"] = {
            ...
            "tmdb_type": result.get("tmdb_type"),
...
def _run_summary_material_closure(artifacts: dict[str, Any], material_missing: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": {
            ...
            "tmdb_type": target_metadata.get("tmdb_type") or metadata_section.get("tmdb_type"),
...
def _post_daily_schedule_notification_webhook(payload: dict[str, Any], url: str, *, timeout: float = 10.0) -> dict[str, Any]:
    ...
    try:
        ...
        if not result["ok"]:
            result["error"] = f"webhook returned HTTP {response.status_code}"
...
def _daily_schedule_delivery_audit(payload: dict[str, Any], *, webhook_url: str | None) -> dict[str, Any]:
    notification_payload = payload.get("notification_payload") if isinstance(payload.get("notification_payload"), dict) else {}
    delivery_result = payload.get("delivery_result") if isinstance(payload.get("delivery_result"), dict) else {}
    file_delivery = delivery_result.get("file_delivery") if isinstance(delivery_result.get("file_delivery"), dict) else {}
    webhook_delivery = delivery_result.get("webhook_delivery") if isinstance(delivery_result.get("webhook_delivery"), dict) else {}
    summary_file = payload.get("summary_file")
    payload_fingerprint = hashlib.sha256(json.dumps(notification_payload, ensure_ascii=False, sort_keys=True).encode("utf-8")).hexdigest() if notification_payload else None
    retry_argv = ["python3", "ptcli.py", "daily-schedule", "--write-summary", "--write-notification", "--json"]
    if summary_file:
        retry_argv.extend(["--summary-output-dir", str(Path(str(summary_file)).expanduser().parent)])
    if webhook_url:
        retry_argv.extend(["--notification-webhook-url", webhook_url])
    blockers = _string_list(delivery_result.get("blockers"))
    return {
        "kind": "ptcli.daily_schedule.delivery_audit",
        "ready": bool(delivery_result.get("ok")),
        "status": delivery_result.get("status"),
        "mutates_state": False,
        "uploads": False,
        ...
        "payload_ref": "notification_payload",
        "summary_file": summary_file,
        "channels": {
            "file": {
            ...
            "webhook": {
                ...
                "url": webhook_delivery.get("url") or webhook_url,
                "status_code": webhook_delivery.get("status_code"),
                "error": webhook_delivery.get("error"),
            },
        },
        "retry": {
            ...
            "reason": "delivery_only_retry_no_upload",
        },
        "continue_when": "delivery_audit.ready=true and delivery_handoff.submission_ready=true",
        "stop_when": ["delivery_audit.blockers is not empty", "delivery_audit.mutates_state=true"],
        "blockers": blockers,
        "next_actions": _daily_schedule_delivery_audit_next_actions(blockers, delivery_result),
    }


def _daily_schedule_delivery_audit_next_actions(blockers: list[str], delivery_result: dict[str, Any]) -> list[str]:
    if blockers:
        return ["Rerun delivery_audit.retry.argv after fixing file permissions or webhook configuration; this does not upload or submit candidates."]
    if delivery_result.get("channel_attempted"):
        return ["Read delivery_audit.channels and delivery_handoff.submission_handoff before asking the user to approve any candidate submission."]
    return ["Configure --write-notification or --notification-webhook-url so daily candidates are actually handed to an AI/IM/webhook consumer."]
...
def _summary_check_print_shell(payload: dict[str, Any]) -> int:
    ...
    fields = {
        ...
        "PTCLI_DAILY_CANDIDATE_SAFE": daily_candidate_targets.get("safe_count"),
...
def _media_info_chain_detail_shell_fields(prefix: str, media_info_chain: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _screenshot_chain_detail_shell_fields(prefix: str, screenshot_chain: dict[str, Any]) -> dict[str, Any]:
    return {
        f"{prefix}_LOCAL_COUNT": screenshot_chain.get("local_screenshot_count"),
        f"{prefix}_IMAGE_HOST_COUNT": screenshot_chain.get("image_host_count"),
        f"{prefix}_DESCRIPTION_COUNT": screenshot_chain.get("description_image_count"),
...
def _summary_check_readiness_shell_fields(readiness_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    media_info_chain = material_description_evidence.get("media_info_chain") if isinstance(material_description_evidence.get("media_info_chain"), dict) else {}
    screenshot_chain = material_description_evidence.get("screenshot_chain") if isinstance(material_description_evidence.get("screenshot_chain"), dict) else {}
    ...
    return {
        ...
        "PTCLI_READINESS_DAILY_CANDIDATE_SAFE": daily_candidate_targets.get("safe_count"),
        ...
        "PTCLI_READINESS_DAILY_CANDIDATE_APPROVAL_READY": _shell_bool(readiness_summary.get("daily_candidate_approval_ready")) if readiness_summary.get("daily_candidate_approval_ready") is not None else None,
        "PTCLI_READINESS_DAILY_CANDIDATE_APPROVAL_SAFE": readiness_summary.get("daily_candidate_approval_safe_count"),
        ...
        **_media_info_chain_detail_shell_fields("PTCLI_READINESS_MATERIAL_DESCRIPTION_MEDIA_INFO_CHAIN", media_info_chain),
        ...
        **_screenshot_chain_detail_shell_fields("PTCLI_READINESS_MATERIAL_DESCRIPTION_SCREENSHOT_CHAIN", screenshot_chain),
...
def _summary_check_target_material_chain_shell_fields(target_material_chain: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **_media_info_chain_detail_shell_fields("PTCLI_TARGET_MATERIAL_CHAIN_MEDIA_INFO", media_info_chain),
        ...
        **_screenshot_chain_detail_shell_fields("PTCLI_TARGET_MATERIAL_CHAIN_SCREENSHOT", screenshot_chain),
    }
...
def _summary_check_resume_material_shell_fields(resume_state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "PTCLI_RESUME_MATERIAL_METADATA_TMDB_TYPE": metadata.get("tmdb_type"),
...
def _summary_check_material_shell_fields(material_diagnostics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "PTCLI_MATERIAL_METADATA_TMDB_TYPE": metadata.get("tmdb_type"),
        ...
        "PTCLI_MATERIAL_IMAGE_HOST_STATUS": image_host.get("status"),
        "PTCLI_MATERIAL_IMAGE_HOST_SKIPPED": _shell_bool(image_host.get("skipped")) if image_host.get("skipped") is not None else None,
        "PTCLI_MATERIAL_IMAGE_HOST_MESSAGE": image_host.get("message"),
        "PTCLI_MATERIAL_IMAGE_HOST_BLOCKERS": "|".join(_string_list(image_host.get("blockers"))),
        "PTCLI_MATERIAL_IMAGE_HOST_HOST": image_host.get("host"),
        "PTCLI_MATERIAL_IMAGE_HOST_COUNT": image_host.get("count"),
        "PTCLI_MATERIAL_IMAGE_HOST_REQUESTED_COUNT": image_host.get("requested_count"),
        ...
        **_media_info_chain_detail_shell_fields("PTCLI_MATERIAL_DESCRIPTION_MEDIA_INFO_CHAIN", media_info_chain),
        ...
        **_screenshot_chain_detail_shell_fields("PTCLI_MATERIAL_DESCRIPTION_SCREENSHOT_CHAIN", screenshot_chain),
