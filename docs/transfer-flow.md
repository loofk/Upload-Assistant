...
`material_diagnostics.description.completeness` 会汇总 MTEAM 描述是否包含 PTGen/Douban 文本、IMDb/TMDb/Douban 外链、MediaInfo/BDInfo、截图 BBCode 和图床截图覆盖率；`--print-shell` 会导出 `PTCLI_MATERIAL_DESCRIPTION_COMPLETE`、`PTCLI_MATERIAL_DESCRIPTION_COMPLETENESS_MISSING` 和 `PTCLI_MATERIAL_DESCRIPTION_COMPLETENESS_CHECKS`。
