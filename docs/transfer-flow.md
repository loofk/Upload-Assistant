...
`readiness_summary` 会同步提升 materials 域的 `material_missing`、`material_upload_gates` 和 `material_upload_blockers`，`summary-check --print-shell` 对应导出 `PTCLI_READINESS_MATERIAL_MISSING`、`PTCLI_READINESS_MATERIAL_UPLOAD_GATES` 和 `PTCLI_READINESS_MATERIAL_UPLOAD_BLOCKERS`，让盒子脚本无需解析完整 diagnostics 就能优先处理素材/描述门禁。
