...
- `check_and_submit_final_report` 是 `source_url_check_and_submit` 的最新首选出口，会把 `verdict=submitted_poll` / `duplicate_stopped` / `blocked`、`recommended_call`、重复种、确认状态、job endpoint、`complete_when` 和 `stop_when` 压成固定字段；OpenClaw/Hermes 应先读它，再按需下钻 `check_and_submit_gate` 和 `manual_retorrent_sequence`。
