...
- Do not pause or cancel while `image_upload` or `target_upload` is actively issuing its external write; the API rejects mid-step interruption. Use step mode or `stop_after_step` to stop at a safe boundary. A crashed/expired upload attempt is converted to reconciliation instead of being auto-retried.
- For `target_upload_outcome_unknown`, `image_upload_outcome_unknown`, or `downloader_partial_add_requires_reconciliation`, use the exact persisted `resume_state.reconciliation` template and latest attempt. Generic resume, retry, and replay are hard-rejected. Only an operator may set `confirmed=true` with the allowed decision, an RFC3339 observation time, and the lowercase SHA-256 of reviewed evidence. Never invent an evidence hash. Target `verified_not_applied` retries only after a fresh duplicate gate and explicit upload confirmation; target `verified_uploaded` binds the target torrent id and submitted torrent SHA-256. Image `verified_uploaded` binds the server-generated pending evidence SHA-256 and only persists the known remote result. Neither recovery path repeats the upload.
