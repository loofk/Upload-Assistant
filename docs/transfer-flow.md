...
`requested_actions` / `effective_actions` 会同时写入顶层和 `summary`，其中 `requested_actions.uploaded_torrent_id` 与 `requested_actions.uploaded_torrent_file` 可区分“按新种 ID 下载恢复”和“已有本地新种文件恢复”。
