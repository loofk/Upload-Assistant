...
后续实现按“live 闭环优先、素材质量其次、扩站瘦身最后”的顺序推进：源种下载/QB 注入/目标站上传/上传后新种做种和规则门禁必须先能返回明确 complete/blockers；视频截图、图床上传、IMDb/TMDb/豆瓣补全、PTGen/Douban 描述、MediaInfo/BDInfo 属于目标上传前的关键素材门禁；更多中文 PT 站点适配、无关 legacy 代码裁剪和体验细节在闭环稳定后再扩展。`material_generation` 中 MediaInfo/BDInfo、截图和图床上传 JSON 会附带 `*_evidence`，记录 path、exists、is_file/is_dir、size_bytes 和 sha1，方便 AI 或盒子脚本判断素材是否真实落盘、是否可复用续跑。
