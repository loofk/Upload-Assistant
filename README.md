...
- `sites --json` 和 `/v1/sites` 暴露每个站点的 `source_info`、`source_info_adapter`、`source_download`、`source_download_adapter`、`credential_requirements`、`target_upload`、`full_live_closure_to_mteam` 能力，并提供 `adapter_contract`、`extension_plan`、`extension_checklist`、`extension_validation_matrix`、`extension_handoff`、`tracker_rollout_handoff` 和 `adapter_extension_final_report`，把新增中文 PT 站点时缺少的 source adapter、download adapter、target upload adapter、policy profile、参考流、验证端点、必需返回证据、站点规则配置契约、下一步实现动作、源站/目标站 rollout 优先级和最终 adapter-ready verdict 结构化给 AI；扩展站点前应优先读取 `adapter_extension_final_report`。
