...
- `sites --json` 和 `/v1/sites` 暴露每个站点的 `source_info`、`source_info_adapter`、`source_download`、`source_download_adapter`、`credential_requirements`、`target_upload`、`full_live_closure_to_mteam` 能力，并提供 `extension_plan` / `extension_checklist`，把新增中文 PT 站点时缺少的 source adapter、download adapter、target upload adapter、policy profile 和下一步实现动作结构化给 AI。
