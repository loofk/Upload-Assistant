...
- `goal_progress.evidence.daily_candidates.daily_candidate_goal_final_report` 是每日 10 条候选目标的总控首读报告，会把当前 schedule 配置、已有 schedule/refill summary、目标数量、ready/safe/shortfall/pending 计数、交付状态、审批要求和唯一 `recommended_call` 压成一个对象；只有 `report_allowed=true`、`target.ready_target_met=true`、`delivery.delivered=true`、`blockers=[]` 时，AI 才能把“今日候选已达标并已交付”报告给用户，提交具体候选仍必须另行取得用户审批和 `confirm_upload=true`。
