...
`resume_state.materials.recovery_hints` 也会保留具体的描述缺口，例如 `description.ptgen_description`、`description.screenshot_bbcode` 和 `description.screenshot_coverage`；这些缺口来自 target preparation missing、payload review 的 description completeness recovery_missing，以及 target preflight 的 `missing` / `description_missing`。其中缺本地截图会触发 `--generate-screenshots`，缺图床覆盖会触发 `--upload-screenshots`，仅缺描述 BBCode 时会优先重新生成目标包描述；高层 `retorrent --execute` 也会把这些 recovery hints 提升到 `readiness_summary.material_recovery`，供 `automation_action` 和 `next_command` 判断使用。
