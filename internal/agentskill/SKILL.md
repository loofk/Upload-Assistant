...
- For `target_upload_outcome_unknown` or `downloader_partial_add_requires_reconciliation`, use the exact persisted `resume_state.reconciliation` template and latest attempt. Generic resume, retry, and replay are hard-rejected. Only an operator may set `confirmed=true` with the allowed decision, an RFC3339 observation time, and the lowercase SHA-256 of reviewed evidence. Never invent an evidence hash. `verified_not_applied` retries only after a fresh duplicate gate and explicit upload confirmation. `verified_uploaded` additionally requires the observed target torrent id and exact submitted torrent SHA-256; the worker performs a fresh read-only target search, persists a recovery-marked receipt, and never repeats the upload.
