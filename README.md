...
- `--write-summary` 会写出带 `automation_handoff` 的 summary JSON，其中 `resume_state.next_stage` / `resume_state.next_command` 可供 agent 或脚本直接续跑，`automation_handoff` 则给出检查和执行续跑的 `summary-check` command/argv。
- `summary-check --json` 会暴露 `automation_handoff`、`flow_diagnostics`、`credential_requirements`、`source_mode`、`target_mode`、`automation_reason` 和逐条标注可执行性的 `candidate_commands`；`--print-next-command` 可只输出下一条安全续跑命令，`--print-next-argv` 可输出对应 argv JSON；`--print-shell` 可输出 `PTCLI_*` shell 变量（含 `PTCLI_SOURCE_MODE` / `PTCLI_TARGET_MODE` / `PTCLI_AUTOMATION_REASON` / `PTCLI_FLOW_READY` / `PTCLI_CREDENTIAL_REQUIREMENTS` / `PTCLI_RUNNABLE_COMMAND_COUNT`）；`--run-next-command` 可直接执行下一条受限的 `ptcli.py` 续跑命令。
