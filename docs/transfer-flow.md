...
所有 `--write-summary` 写出的 summary 顶层都会带 `automation_handoff`，对应命令本次 JSON 返回也会同步给出该字段；它内含 `summary-check --json`、`--print-next-command`、`--print-next-argv`、`--print-first-runnable-command`、`--print-first-runnable-argv`、`--print-shell` 和 `--run-next-command` 的 command/argv，盒子脚本可以直接读取 stdout 或落盘 summary 继续检查/续跑，无需自己拼 summary 路径。`summary-check --json` 也会按传入的 summary 路径补齐同样的 `automation_handoff`，即使输入 summary 是旧 schema、未知 kind 或文件缺失，也能返回可审计的下一步检查入口。
