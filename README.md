...
- `/v1/deployment/check` 会输出 `mounts`、`qbit`、`daily_candidates` 和 `agent_summary`：AI 可以直接判断 config/cookies/tmp/job/downloads 挂载是否就绪、qBittorrent 是否配置、`PTCLI_DAILY_CANDIDATE_SCHEDULES` 是否已提供每日候选计划；每日候选未配置只作为 warning，不阻塞手动转种 API。
