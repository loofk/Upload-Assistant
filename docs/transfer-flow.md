...
`summary-check` 还会暴露 `next_command_source`、`next_command_subcommand`、`next_command_run_allowed` 和 `next_command_run_blocker`；`--print-shell` 对应导出 `PTCLI_BLOCKERS`、`PTCLI_MISSING_ARTIFACTS`、`PTCLI_MISSING_CLOSURE_AUDIT`、`PTCLI_NEXT_COMMAND_SOURCE`、`PTCLI_NEXT_COMMAND_SUBCOMMAND`、`PTCLI_NEXT_COMMAND_RUN_ALLOWED` 和 `PTCLI_NEXT_COMMAND_RUN_BLOCKER`，盒子脚本可以区分命令来自 `resume_state` 还是 fallback，并在调用 `--run-next-command` 前先判断这条命令是否属于受限自动执行白名单。
