...
`retorrent --execute` 是高层闭环命令，会默认执行上传后目标站种子下载和 qBittorrent 注入。顶层 JSON 只有在 `closure.complete=true` 且 pipeline ready 时才返回 `status: complete`；否则返回 `status: blocked` 和 blockers。source 侧闭环可由“源种下载 + QB 注入 + 等待完成”证明，也可由“已有 `--path` 能匹配 QB 任务”证明。`pipeline` / `target-upload` 保留显式参数，便于拆分排障和人工复核。
