...
- `job_lifecycle_final_report` 是 job status、summary 和 list 的统一首读出口，会把当前 job 压成 `verdict=poll` / `preview_resume` / `execute_resume_after_review` / `read_summary` / `stop_duplicate` / `blocked` / `report_complete`，并固定暴露 `primary_next_step`、`recommended_call`、`dry_run_request`、`execute_request`、poll/resume/summary `sequence`、`complete_when`、`stop_when`、安全说明和 blockers；AI 应先读它，再下钻 `job_control_summary`、`resume_final_report`、`job_final_report`、`live_user_report`、`manual_retorrent_final_report` 或 `recovery_handoff`。`job_final_report` 仍是判断是否允许向用户报告完成的最终证据出口。
...
- `live_validation_completion_audit` 是首单 live 验证的最后一致性审计，会交叉检查 `live_user_report`、`live_validation_final_report`、`seedbox_live_validation_completion_report`、`closure_summary`、`qbit_enforcement_summary`、`policy_execution_report` 和素材证据；AI 最终向用户报告完成前必须最后读取它，且只有 `report_allowed=true`、`failed_checks=[]`、`missing_evidence=[]`、`blockers=[]` 时才能宣告真实闭环完成。
- `goal_progress.evidence.live_validation.live_recovery_final_report` 是 `/v1/goal/progress` 对已提交 live job 的首选恢复总控，会把 poll、dry-run resume、用户确认后 execute resume、read summary、duplicate stop 和 blockers 压成 `verdict`/`action`/`recommended_call`/`dry_run_request`/`execute_request`；`goal_progress.next_step` 会优先继承它，AI 应先读它选择下一步，再用 `live_validation_completion_audit` 证明是否能向用户报告完成。`goal_progress.evidence.live_validation.seedbox_live_validation_final_report` 仍作为首单 live 验证汇总报告，负责串联 doctor 可提交、已提交 job、最终审计和用户报告条件。
- `policy_application_handoff` 是站点策略默认值落到具体 job 请求后的审计出口，会在 job status、summary、list、`agent_decision`、`workflow_context`、`policy_execution_report` 和候选提交 handoff 中固定暴露 `request_patch`、`applied_request_fields`、`missing_request_fields`、`protected_fields`、`missing_confirmations`、QB/做种/规则证据引用和下一步请求；OpenClaw/Hermes 必须先确认 `policy_application_handoff.ready=true` 且缺失字段为空，再继续 live 上传或向用户报告闭环。
- `policy_execution_final_report` 是规则执行、限速和做种要求的最终安全出口，会在 job status、summary、list、`agent_decision` 和 `workflow_context` 中固定暴露 `ready_for_live`、`verdict`、站点规则确认、qBittorrent 上传/下载限速审计、做种要求、runtime contract、推荐调用和 blockers；OpenClaw/Hermes 应先读它，再按 `read_order` 复核 `policy_application_handoff`、`policy_execution_report`、`qbit_execution_gate`、`qbit_enforcement_summary` 和 `qbit_limit_audit`。
- `site_policy_final_report` 是 `/v1/site-policies` 的规则/限速/做种配置总控出口，会把 `policy_repair_gate`、`policy_execution_targets`、`policy_runtime_contract`、`policy_execution_contract` 和 `policy_application_handoff` 压成 `ready_for_live`、`report_allowed`、`rate_limits`、`seeding`、`rules`、`live_request_template` 和 `recommended_call`；AI 只有在 `report_allowed=true` 时才应把当前站点策略视为可进入 live preflight。
...
- `manual_retorrent_final_report` 是手动源链接转种/每日候选提交后正式转种 job 的总控报告，会在 job status、summary、list、`agent_decision`、`workflow_context`、`candidate_submission_handoff`、`candidate_submission_summary`、`candidate_submit_followup`、`candidate_submit_sequence` 和每日批次 `submitted_jobs` 中串联重复检查、`policy_execution_final_report`、`material_preparation_final_report`、目标上传、closure、live gate、`live_recovery_final_report` 和最终验证报告，固定暴露 `verdict`、`report_allowed`、`live_recovery`、`recommended_call`、`complete_when`、`stop_when`、blockers 和下一步动作；AI 应先读它判断是停止重复种、请求确认、补规则、补素材、继续上传闭环、轮询还是向用户报告完成，并优先使用其内联的 `live_recovery_final_report.recommended_call` 处理 poll、dry-run resume、用户确认后 execute resume 或最终 summary 读取。
- `daily_candidate_final_report` 是每日候选批次的最终报告出口，会在 schedule job、batch status 和 job list 中固定暴露 `verdict`、目标/已选/可提交/短缺/待轮询数量、notification、approval、submission、shortfall recovery、audit refs 和下一步调用；候选推送 payload 还会暴露 `candidate_field_completeness`，逐条检查源站、目标站、种子 ID、标题、大小、发布时间、促销/free、IMDb/TMDb/豆瓣 metadata、查重、可下载性、推荐理由、风险和提交入口是否齐备。AI 应先读这些字段再决定报告候选、补扫短缺、轮询任务或请求用户批准提交候选。
- `candidate_field_contract_report` 是每日候选 digest/publish payload 的字段合同审计，会把源站、种子 ID、标题、大小、发布时间、促销/free、IMDb/TMDb/豆瓣任一 metadata、目标查重、可下载性、推荐理由、风险/阻塞原因和提交入口逐条压成 `checks` 与 `missing_by_source_id`；只有 `report_allowed=true` 时，AI 才能把候选推送视为字段完整。
- `goal_progress.evidence.daily_candidates.daily_candidate_goal_final_report` 是每日 10 条候选目标的总控首读报告，会把当前 schedule 配置、已有 schedule/refill/run-and-deliver summary、目标数量、ready/safe/shortfall/pending 计数、交付状态、审批要求、`completion_evidence` / `missing_evidence` 和唯一 `recommended_call` 压成一个对象；当 `daily_candidate_run_loop_report` 存在时，goal progress 会优先继承它的 `action` 和 `recommended_call`，让 AI 直接决定补扫、轮询、交付或等待审批。只有 `report_allowed=true`、`target.ready_target_met=true`、`delivery.delivered=true`、`missing_evidence=[]`、`blockers=[]` 时，AI 才能把“今日候选已达标并已交付”报告给用户，提交具体候选仍必须另行取得用户审批和 `confirm_upload=true`。
- `daily_candidate_delivery_final_report` 是每日候选 schedule 交付的最终报告出口，会在 schedule job、`schedule_digest`、`notification_payload`、`delivery_handoff` 和 `daily_candidate_delivery_plan` 中固定暴露 `verdict`、发布/提交状态、通知载荷引用、推荐调用、安全边界和 blockers；AI 应先读它判断是仅发布候选、请求用户审批提交、补短缺、轮询还是停止，且 `publish_notification` 永远不代表上传或提交种子。
- `daily_candidate_config_final_report` 是每日 10 条候选部署配置的首读出口，会在 `deployment_check`、`deployment_handoff`、`agent_handoff`、`deployment_runbook` 和 `goal_progress.evidence.daily_candidates` 中固定暴露可复制进 `.env` 的 `PTCLI_DAILY_CANDIDATE_SCHEDULES` 行、JSON、sha256、compose one-shot/daemon 命令、API smoke checks 和安全边界；AI 应先用它配置/验证 scheduler，再读取候选 digest。
- `daily_candidate_operational_final_report` 是每日 10 条候选运营闭环的首读出口，会在 schedule job、run-and-deliver 和 goal progress 中聚合目标/已选/ready/可提交/短缺/待轮询/完成计数、投递状态、审批要求、唯一 `recommended_call` 和安全边界；OpenClaw/Hermes 应优先读取它，再按 `action=publish_digest` / `request_user_approval` / `poll_jobs` / `refill_shortfall` / `report_complete` 继续，且 run-and-deliver 本身永不提交候选或上传种子。`daily_candidate_run_loop_report` 是 run-and-deliver 响应里的循环控制短路径，会把运营报告、目标达标、交付证据和下一步调用压成 `action=refill_shortfall` / `request_user_approval` / `poll_jobs` / `publish_digest` / `report_complete`，便于 AI 在一次运行后直接决定补扫、轮询、交付或等待审批。
- `daily_candidate_target_fulfillment_report` 是每日 10 条是否真正达标的短路径，会在 schedule job、run-and-deliver、`schedule_digest.push_payload`、`notification_payload`、`delivery_handoff` 和 `daily_candidate_delivery_plan` 中固定暴露 target/ready/safe/shortfall/pending 计数、短缺补扫请求、用户审批请求和安全边界；AI 应先读它判断是补足短缺、轮询待完成 job、发布 digest，还是只在用户明确批准后提交候选。
- `daily_scheduler_final_report` 是 Docker/盒子常驻 `daily-scheduler` 的运行首读出口，会在 `daily-scheduler --once` 输出、连续模式 `scheduled_run` 事件和 `ptcli-daily-schedule-summary.json` 中固定暴露下一次计划、上次推送交付、summary 文件、target fulfillment、推荐调用和安全边界；AI 可只读它判断上次是否已把候选交给文件/webhook、是否需要补足 10 条、轮询候选 job 或等待用户批准。
- `readiness_bundle.seedbox_live_runbook_final_report` 是盒子首单 live 验证的首读操作报告，会把当前 verdict、第一步 `recommended_call`、doctor、check-and-submit、poll/resume、最终报告字段和 `must_not_report_complete_until` 固定到一个对象；`readiness_bundle.live_execution_package` 则是详细执行总包，会把 doctor、summary-check、查重提交、poll/resume 和最终 `live_validation_completion_audit` 读取顺序固定到 `steps`/`run_order`/`report_contract`，AI 应按它执行 U2/CHD -> MTEAM 首单验证。`readiness_bundle.seedbox_live_post_submit_report` 是 doctor 通过后、提交 `source_url_check_and_submit` 之后的首读追踪短路径，会把提交、轮询、dry-run/execute resume、最终 summary/audit 读取顺序和 stop 条件压成一个对象，避免 AI 在首单 live job 已创建后手工拼 poll/resume/summary 调用。
- `readiness_bundle.seedbox_live_validation_start_report` 是首单 live 验证开始前的安全闸口，会把 `doctor_call`、`summary_check_call`、`submit_call_template`、`post_submit_calls`、`final_audit_contract`、`must_not_submit_until` 和 `must_not_report_complete_until` 压成一个首读对象；它的 `report_allowed` 固定为 `false`，只允许 AI 在 `start_allowed=true` 且用户确认后启动 doctor，并且只有 summary-check 明确允许后才进入 check-and-submit。
...
- `sites --json` 和 `/v1/sites` 暴露每个站点的 `source_info`、`source_info_adapter`、`source_download`、`source_download_adapter`、`credential_requirements`、`target_upload`、`full_live_closure_to_mteam` 能力，并提供 `adapter_contract`、`extension_plan`、`extension_checklist`、`extension_validation_matrix`、`extension_handoff`、`tracker_rollout_handoff`、`adapter_extension_final_report` 和 `site_extension_readiness_final_report`，把新增中文 PT 站点时缺少的 source adapter、download adapter、target upload adapter、policy profile、参考流、验证端点、必需返回证据、站点规则配置契约、下一步实现动作、源站/目标站 rollout 优先级和最终 adapter-ready verdict 结构化给 AI；扩展站点前应优先读取 `site_extension_readiness_final_report`，它会聚合角色要求、规则配置必填字段、按 tracker 缺失项、验收证据和 seedbox live validation 下一步。
...
- `ptcli serve` 默认 `PTCLI_MAX_CONCURRENT_JOBS=1`，同一时间只执行一个长耗时 job，其余保持 `queued`；`/v1/jobs` 会返回 `queue.max_concurrent_jobs`、`running_count` 和 `queued_count`，便于 agent 判断排队情况。`/v1/jobs.daily_candidate_batch_summary` 会把每日候选 job 与候选提交后生成的正式转种 job 聚合成批次视图，暴露 `candidate_job_count`、`submitted_retorrent_job_count`、`unsubmitted_safe_count`、`retorrent_status_counts`、`retorrent_action_counts`、`items[].submit_requests`、`items[].submitted_jobs`、`blockers` 和 `next_actions`；其中 `items[].submitted_jobs[].job_final_report` 是已提交转种 job 的最终首读结论，旁路暴露 `job_final_verdict` 和 `job_report_allowed`，让 AI 不用再跳转到单个 job 才能判断该候选是继续轮询、恢复执行、停止重复种还是可以报告完成；`/v1/jobs.daily_candidate_batch_gate` 会进一步把同一批次压成 `action=submit_candidate` / `poll_submitted_jobs` / `resolve_blockers` / `complete` / `inspect_empty`、首个提交请求、首个已提交 job、推荐工具/端点/请求和 stop 条件；`/v1/jobs.daily_candidate_submission_plan` 是刷上传批次的提交短路径，会聚合所有尚未提交的安全候选、`first_submit_request`、`submit_requests[]`、`shortfall_recovery`、`hard_blockers` 与 `shortfall_blockers`，让 OpenClaw/Hermes 能先提交可安全执行的候选，再按 `shortfall_recovery.recommended_request` 补足每日 10 条目标；`/v1/jobs.daily_candidate_execution_summary` 是提交后的结果短路径，会把正式转种 job 分成 `complete_jobs`、`running_jobs`、`blocked_jobs`，暴露 `remaining_submit_count`、`ready_shortfall_count`、`completion_ratio`、下一步工具/端点/请求和 stop 条件；`/v1/jobs.daily_candidate_submitted_jobs_report` 是已提交正式转种 job 的批次级追踪首读口，会把 `continue_submitted_job` / `poll_submitted_jobs` / `report_complete` / `submit_remaining`、首个 submitted job、唯一推荐调用、报告允许条件和安全边界压成短路径；`/v1/jobs.daily_candidate_refill_plan` 是补足 10 条的控制面，会暴露 `ready_shortfall_count`、`scan_count`、`max_scan_count`、`scan_exhausted`、已覆盖/已提交/阻塞 source ids、`pagination_supported=false`、排重策略和重跑请求，避免 AI 重复提交同一候选或假装已有分页能力。若只想读取每日候选批次状态，可调用只读入口 `/v1/jobs/candidates/daily/batch?source_tracker=U2&target=MTEAM`，它返回同一份 `daily_candidate_batch_summary` / `batch_summary`、`daily_candidate_batch_gate` / `batch_gate`、`daily_candidate_submission_plan` / `submission_plan`、`daily_candidate_execution_summary` / `execution_summary`、`daily_candidate_submitted_jobs_report` / `submitted_jobs_report` 和 `daily_candidate_refill_plan` / `refill_plan` 别名，不会创建任务、下载或上传。
...
- 转种 job 状态和 summary 也会暴露 `policy_coverage`、`policy_handoff` 和 `policy_application_handoff`；`policy_handoff` 会把源站/目标站规则页、人工审查 fingerprint、上传/下载限速、做种要求、缺失策略字段和下一步工具压缩成固定路径。`site_policies.policy_readiness_summary` 是站点策略配置的首选短路径，会汇总 ready/phase、缺失限速、缺失做种要求、缺失或占位 fingerprint、首个 blocker、推荐下一步和可复制的 `config.preferred_patch`；`site_policies.policy_repair_gate` 会进一步把策略修复压成 `action=review_rules` / `edit_config` / `resolve_blockers` / `ready_for_live_preflight`、人工审查要求、首个 blocker、preferred patch、手动步骤和重跑请求，作为 AI 修复站规配置前的首选控制面；`site_policies.policy_config_handoff` 会把规则审查请求、限速/做种配置 patch、fingerprint patch 合并顺序、逐站点缺口和复核请求压成固定路径，明确 `rule_review_fingerprint` 只能来自人工审查，AI 不得自动生成后静默应用；`site_policies.policy_execution_handoff` 会进一步把 QB 限速、做种要求、转种过滤规则、规则 obligation、配置模板和继续/停止条件压成 AI 可直接读取的执行 handoff；`site_policies.policy_application_handoff` 会给出 live 请求必须合并的 `request_patch`、受保护限速字段和提交覆盖模板。当 `accept_rules` / `confirm_upload` 已齐但源站或目标站缺少 fingerprint、限速、做种要求，或 job 请求没有应用策略 patch 时，`agent_decision` 会阻断继续 live 上传。
...
- `/v1/site-policies/rule-review` 是人工站规审查后的配置辅助入口：`policy_repair_gate.action=review_rules` 时会把 `recommended_tool` 指向 `site_policy_rule_review`，并给出 `rule_review_request`；未提交人工证据时也会返回 `rule_review_package`，列出待审站点、规则 URL、scope、提交模板、审后配置和验证顺序。只有显式传入 `rules_reviewed=true`、`reviewer` 和 `reviewed_at` 才会生成每站点 `rule_review_fingerprint`、`config_patch.structured_patch` / `flat_patch`、合并好限速/做种模板和 fingerprint 的 `merged_config_patch`、`rule_review_final_report`、`config_apply_final_report.copyable_config` 和 `after_edit` 重跑请求。`rule_review_final_report` 是首读出口，会明确 `verdict=needs_manual_review` / `patch_ready`、合并 `policy_config_handoff.preferred_patch` 与 fingerprint patch 的顺序、不得自动套用的安全边界、以及复制 `merged_config_patch.structured_patch` 后必须重跑 `/v1/site-policies` 的完成条件；`copyable_config` 会额外给出可粘贴进 `data/config.py` 的 `python_update_snippet`、`json_patch` 和 `patch_sha256`。它不联网、不编辑配置、不替用户确认规则，只把人工审查证据转换成可审计 patch。
- retorrent/manual job 会在未显式传入限速时，从 `PTCLI.SITE_POLICIES` 自动补齐 `qbit_download_limit`、`qbit_upload_limit`、`uploaded_qbit_upload_limit`、`uploaded_qbit_download_limit`；job 状态、summary 和 `agent_decision.policy_qbit_defaults` 会记录哪些值来自站点策略、哪些值由请求覆盖，`qbit_plan` 会汇总 source/uploaded 两侧最终的分类、标签、上传/下载限速和来源，`qbit_limit_audit` 会把计划限速与实际 qBittorrent 注入结果中的 `rate_limits.calls` 对齐，区分 `applied`、`pending` 和 `mismatch`；`qbit_handoff` 会把两侧的分类、标签、限速来源、审计状态和下一步动作压缩成 AI 可直接读取的 QB 执行摘要，其中 `enforcement_handoff.roles[]` 会逐项列出 source/uploaded 的预期限速、观察到的限速、是否缺注入证据、是否需要修复限速，以及下一步 resume/get-summary 建议；`qbit_enforcement_summary` 会把同一审计再压缩成 `expected_role_count`、`applied_role_count`、`pending_roles`、`mismatch_roles`、`recommended_tool` 和 blockers；`qbit_rate_limit_repair_plan` 会在 pending/mismatch 时把 source/uploaded 的期望限速、分类/标签、`request_patch`、`dry_run_request`、`execute_request`、`qbit_apply_limits_calls`、`recommended_call` 和完成条件压成可直接审阅的修复计划，直接限速修复请求会携带父 `job_id`；`qbit_final_report.repair` 会内联同一组 repair 角色、直接 `qbit_apply_limits` 调用和 summary verification call，AI 只读 `qbit_final_report` 就能执行限速修复短闭环；`qbit_apply_limits` 响应里的 `qbit_rate_limit_repair_report` 会把本次修复、角色、hash、请求限速、是否已应用和后续 `get_job_summary` 验证调用压成短路径；`qbit_execution_gate` 会把这些 QB 限速/注入/做种证据进一步压成 `ready`、`action`、`first_blocker`、`dry_run_request`、`execute_request`、`read_order` 和 `continue_when`/`stop_when`，作为 AI 判断刷上传自动化是否可继续的最短路径；`policy_execution_report` 会进一步把规则接受状态、做种要求、站点策略默认限速、请求覆盖、QB 计划和实际执行摘要压成 AI 首选短路径，便于 OpenClaw/Hermes 判断“站点规则和限速/做种策略是否已经可继续自动化”；resume job 的 `resume_context.inherited_policy` 也会保留父任务的策略上下文。
...
# 部署后 AI smoke：不联系站点/qBittorrent，聚合 health、tools、manifest、agent_manifest_readiness_report、
# agent_smoke_command_report.commands、agent_smoke_live_validation_handoff、deployment、readiness 和 goal_progress
...
# 每日候选运行并投递：创建候选扫描 job 后写本地通知文件/可选 webhook；不会提交候选或上传
curl -X POST http://127.0.0.1:8080/v1/jobs/candidates/daily/run-and-deliver \
  -H "Content-Type: application/json" \
  -d '{"schedules":[{"name":"u2-to-mteam","source_tracker":"U2","target":"MTEAM","limit":10,"time":"09:00","accept_rules":true}],"write_files":true,"use_env_webhook":true}'
...
手动源链接转种的 AI runbook 固定为：先调用 `source_url_retorrent_preflight` 读取 `ready_to_create_job`、`source_reference`、`target_trackers`、`policy_execution_summary`、`policy_execution_handoff`、`duplicate_check.next_request`、`duplicate_check_handoff`、`one_call_handoff`、`job_creation_handoff.request`、`next_step` 和 `blockers`；该端点不创建 job、不访问 tracker、不访问 qBittorrent，只用于判断源链接解析、部署、规则、确认和素材前置条件是否足以进入任务创建。若 `ready_to_create_job=false`，AI 必须按 `next_step` 修复部署、策略、确认或配置缺口；若 ready，再调用 `readiness_bundle` 复核 `live_readiness.ready_for_manual_retorrent`、`live_readiness.policy_execution_handoff`、`live_test_handoff.policy_execution_handoff` 和 `manual_job_template.request`；再调用 `site_policies` 确认 `ready=true`、`policy_execution_handoff.ready=true`、`policy_application_handoff.ready=true`、`policy_application_handoff.request_patch` 已覆盖 live 请求默认限速、`execution_readiness.ready=true` 与 `rule_obligations.*.ready=true`，其中源站 scope 应为 `download_and_retorrent`、目标站 scope 应为 `upload_and_seed`。推荐的一步式入口是 `one_call_handoff.request` 指向的 `source_url_check_and_submit` / `/v1/jobs/retorrent/from-url/check-and-submit`：它会先同步执行目标站查重，若 `duplicate_check.exists=true` 则返回 `status=blocked`、重复种信息并且不创建 live job；只有 `duplicate_check.searched=true`、`duplicate_check.exists=false`、`submit_if_clear_handoff.ready=true`、`accept_rules=true`、`confirm_upload=true` 时才创建后续 live job 并返回 `job_id`、`status_endpoint` 和 `summary_endpoint`；该响应的首选短路径是 `check_and_submit_gate`，其 `action` 会稳定区分 `poll_job`、`stop_duplicate` 和 `resolve_gate_blockers`，并同步给出 `duplicate_clear`、`submit_ready`、确认状态、推荐工具/端点/请求和 stop 条件；同时读取 `check_and_submit_policy_report` 可以直接确认本次提交是否已继承 `policy_qbit_defaults`、`policy_application_handoff`、`policy_runtime_contract`、`qbit_plan`、`qbit_enforcement_summary` 与缺失/待执行的 qBittorrent 限速角色。需要拆分审计时，也可以调用 `retorrent_check` / `/v1/retorrent/check` 或异步 `retorrent_check_job` / `/v1/jobs/retorrent/check` 执行目标站查重，只有 `duplicate_check.searched=true`、`duplicate_check.exists=false` 且 `submit_if_clear_handoff.ready=true` 时，才把同步结果里的 `submit_if_clear_handoff.request` 提交给 `source_url_retorrent_job`，或把已完成的查重 `job_id` 提交给 `submit_checked_retorrent_job` / `/v1/jobs/retorrent/check/{job_id}/submit`；若 `duplicate_check.exists=true`，必须停止并返回重复种信息。由查重派生的 live job 会在 `check_submission` 中记录查重结果、继承 request、执行覆盖参数和 qBittorrent 限速/分类证据，方便审计 AI 没有绕过查重 gate。任务创建后优先读取 `policy_application_handoff` 与 `job_handoff`：若 `policy_application_handoff.missing_request_fields`、`missing_protected_fields` 或 `missing_confirmations` 非空，必须先按其 `next_actions` 补齐；当 `job_handoff.action=wait` 且 `job_handoff.should_poll=true` 时按 `job_handoff.poll_after_seconds` 轮询 `get_job_status`；当 `job_handoff.action=resume` 时先用 `job_handoff.recommended_request` 预览 `resume_job`；当 `job_handoff.action=stop` 时停止并报告 blockers；当 `job_handoff.action=done` 时读取 `get_job_summary` 并回报闭环证据。完成后仍需最后读取 `live_validation_completion_audit`，并同时参考 `live_user_report`、`seedbox_live_validation_completion_report`、`closure_summary` / `closure_handoff` 作为详细闭环证明；若 `seedbox_live_validation_completion_report.status` 返回 `duplicate_stopped`、`needs_resume`、`blocked`，或 `closure_summary.action` 返回 `stop_duplicate`、`collect_confirmations`、`configure_policy`、`prepare_materials`、`repair_target_payload`、`repair_qbit` 或 `resolve_blockers`，AI 必须按 `seedbox_live_validation_completion_report.recommended_call`、`job_handoff` 或 `closure_summary.next_step` 和 blockers 停止、补资料或续跑；只有 `live_validation_completion_audit.report_allowed=true`、`live_validation_completion_audit.failed_checks=[]`、`live_validation_completion_audit.missing_evidence=[]` 且 `live_validation_completion_audit.blockers=[]` 才能把任务视为真实闭环完成。调用前应读取 `resume_summary`、`materials_handoff.recommended_inputs`、`target_upload_handoff.blockers`、`closure_summary.blockers` 和 `resume_requirements`，按其中的 `missing_confirmations`、`suggested_overrides`、`recommended_inputs`、`dry_run_request`、`execute_request` 和 `allowed_overrides` 补充缺失确认、路径、限速或素材文件；真正续跑前建议优先调用 `job_handoff.recommended_request`、`resume_summary.next_step` 或 `resume_job` 并设置 `dry_run=true` 预览 patched `command_argv`，确认后再用同一组 allowlisted override 去掉 `dry_run` 执行；调用后应读取 `resume_summary`、`resume_audit`、`resume_context.applied_overrides` 和 `material_resolution.covered_recommended_inputs` / `unresolved_recommended_inputs` 判断父子任务关系、override 是否生效以及本次续跑是否覆盖了缺口；未知 override 会被忽略并记录在 `resume_context.ignored_overrides`。
- `check_and_submit_final_report` 是 `source_url_check_and_submit` 的最新首选出口，会把 `verdict=submitted_poll` / `duplicate_stopped` / `blocked`、`recommended_call`、重复种、确认状态、job endpoint、`complete_when` 和 `stop_when` 压成固定字段；OpenClaw/Hermes 应先读它，再按需下钻 `check_and_submit_gate` 和 `manual_retorrent_sequence`；提交出正式转种 job 后，`check_and_submit_job_tracking_report` 会把新 job 的 `manual_retorrent_final_report`、`live_recovery_final_report`、剩余步骤、status/summary/resume endpoint 和唯一推荐调用压成首选追踪短路径，后续 job status/summary 的首选转种总控仍为 `manual_retorrent_final_report.live_recovery_final_report`。

AI 查询普通 job 后可读取 `job_control_summary`，查询 live 验证或转种闭环 job 时应优先读取 `live_recovery_final_report`：`action=poll` 时轮询 `get_job_status`，`action=resume_preview` 时先调用 `dry_run_request`，`action=resume_execute` 时在用户确认后调用 `execute_request`，`action=stop_duplicate/blocked/failed/cancelled` 时停止并报告 blockers，`action=read_summary` 时读取 `get_job_summary` 并优先报告 `live_validation_completion_audit`，同时保留 `live_user_report`、`seedbox_live_validation_completion_report` 与 `closure_summary` 作为审计证据；`job_handoff` 和 `recovery_handoff` 继续作为详细兼容字段。
...
- 站点自动化策略可写在 `config["PTCLI"]["SITE_POLICIES"]` 或顶层 `config["SITE_POLICIES"]`。默认只启用当前参考闭环的保守自动化能力；限速、做种要求和人工审查指纹建议按站点规则自行维护，例如：
  ...
  job status/list/summary 还会暴露 `policy_application_report`，用于压缩判断站点策略是否已经真正应用到当前 live request：`request_patch_applied`、`protected_fields_ok`、`confirmations_ok` 会证明限速/做种策略字段和显式确认都在请求里，`qbit_evidence_ready`、`qbit_pending_roles`、`qbit_mismatch_roles` 会指向后续 QB 限速/做种证据是否还需要 resume 或修复。AI 报告完成前应优先读取 `policy_application_report`、`qbit_enforcement_summary`、`qbit_execution_gate` 和 `live_validation_completion_audit`，再用 `live_user_report` 补充用户可读摘要。
...
- `/v1/summary/check.live_submission_package` 是 doctor summary 通过后的只读提交包：它会把 `source_url_check_and_submit` 的 request/curl、提交后 `get_job_status` 轮询、`resume_job` 预览/执行条件、`get_job_summary` 收尾读取字段和 `live_validation_completion_audit` 最终报告条件固定到一个对象；提交 request 会携带 `live_validation_submission` 来源证明，提交后的 job status/summary/list 会暴露 `live_validation_followup`，让 AI 直接判断下一步是 poll、resume 还是读取最终报告；该字段不会执行上传，AI 仍必须单独调用提交 endpoint，并在 `live_validation_completion_audit.report_allowed=true`、`live_validation_completion_audit.failed_checks=[]`、`live_validation_completion_audit.missing_evidence=[]` 与 `live_validation_completion_audit.blockers=[]` 后才能报告首单 live 验证完成。`/v1/summary/check.live_submission_final_report` 是同一结果的首读报告，优先看 `ready`、`verdict`、`submission`、`post_submit`、`completion_contract` 和 `safety.must_not_report_live_complete`；它只允许 AI 判断“提交/修复/停止”，不会把 doctor 通过误报成 live 完成。
