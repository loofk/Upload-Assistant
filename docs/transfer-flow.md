...
所有 `--write-summary` 写出的 summary 顶层都会带 `automation_handoff`，内含 `summary-check --json`、`--print-next-command`、`--print-next-argv`、`--print-shell` 和 `--run-next-command` 的 command/argv，盒子脚本可以直接读取这个字段继续检查或续跑，无需自己拼 summary 路径。
