...
- `readiness_bundle.seedbox_live_runbook_final_report` 是盒子首单 live 验证的首读操作报告，会把当前 verdict、第一步 `recommended_call`、doctor、check-and-submit、poll/resume、最终报告字段和 `must_not_report_complete_until` 固定到一个对象；`readiness_bundle.live_execution_package` 则是详细执行总包，会把 doctor、summary-check、查重提交、poll/resume 和最终 `live_user_report` 读取顺序固定到 `steps`/`run_order`/`report_contract`，AI 应按它执行 U2/CHD -> MTEAM 首单验证。
