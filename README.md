...
- `goal_progress.evidence.live_validation.seedbox_live_validation_final_report` 是 `/v1/goal/progress` 里的首单 live 验证首读报告，会把未开始、doctor 可提交、job 运行中、需要续跑、可读取 summary、重复停止和完成状态压成 `verdict`/`phase`/`recommended_call`/`report_allowed`；AI 应先读它选择下一步，再用 `live_validation_completion_audit` 证明是否能向用户报告完成。
