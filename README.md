...
- 任务响应中的 `job_handoff.recommended_call` 会把下一步 `tool`、`endpoint`、`method`、`request`、`dry_run_request`、`execute_request`、`requires_user_review` 和 safety/stop 条件聚合成单一对象，便于 OpenClaw/Hermes 按 queued/running/blocked/complete 状态安全轮询、预览恢复、执行恢复或读取 summary。
