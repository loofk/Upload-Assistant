...
- For `target_upload_outcome_unknown` or `downloader_partial_add_requires_reconciliation`, use the exact persisted `resume_state.reconciliation` template and latest attempt. Generic resume, retry, and replay are hard-rejected. Only an operator may set `confirmed=true` with the allowed decision, an RFC3339 observation time, and the lowercase SHA-256 of reviewed evidence. Never invent an evidence hash. A target-upload retry is allowed only after `verified_not_applied`; it still runs a fresh duplicate gate and requires explicit upload confirmation.
