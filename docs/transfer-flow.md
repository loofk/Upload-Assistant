...
当下一步是 `resume-target-package` 但恢复命令覆盖率不完整时，`summary-check` 会返回 `automation_action=complete_material_recovery_command`，并通过 `next_command_run_blocker` 指出第一条未覆盖的素材缺口和缺少的 flags，避免盒子脚本自动执行一个无法真正补齐素材的命令。
