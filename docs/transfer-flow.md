...
`readiness_summary.material_recovery` 会把素材恢复信息压成可审计对象，包含缺失素材键、下一步动作、需要补的命令 flags、可替代的已有文件参数，以及第一条可用的 `resume-target-package` 恢复命令；`--print-shell` 会同步导出 `PTCLI_READINESS_MATERIAL_RECOVERY_*`。
