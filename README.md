...
- `seedbox_live_validation_completion_report` 是盒子首单 live 验证提交 job 后的收尾短路径，会把 `status`（`complete` / `duplicate_stopped` / `running` / `needs_resume` / `blocked`）、`checks`、`missing_evidence`、源/目标 hash、上传后新种路径、qBittorrent 限速/做种证据、下一步工具和 blockers 固定到一个对象；只有 `ready_for_user_report=true`、`missing_evidence=[]` 且 `blockers=[]` 时，AI 才能把首单验证报告为完成。
