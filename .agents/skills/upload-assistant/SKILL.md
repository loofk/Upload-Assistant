...
Before any external probe or controlled live workflow, call `GET /api/v2/readiness/live` (or CLI `readiness live`) with the requested U2/CHD source, MTEAM target, downloader, image host, and screenshot profile. It checks local configuration only, including the native MediaInfo, BDInfo, FFmpeg, FFprobe, and mkbrr binaries. It never contacts a tracker, downloader, image host, or media manager; it always returns `external_calls_performed=false`, `live_upload_authorized=false`, and `resume_state.confirm_upload=false`. Show the operator its blockers, next actions, exact rule fingerprints, and obligation IDs. Never treat `configuration_ready=true` as credential validation, duplicate clearance, network-probe consent, rule acceptance, or upload consent.
