...
func toolDefinitions() []toolDefinition {
	...
	return []toolDefinition{
		...
		{Name: "create_screenshot_profile", Description: "Create an immutable new screenshot configuration revision.", Method: "POST", Path: "/api/v2/screenshot-profiles", RequiredScopes: []string{"config:manage"}, SafetyLevel: "configuration_write", InputSchema: object(map[string]any{"name": stringProperty("Screenshot profile name."), "enabled": map[string]any{"type": "boolean"}, "config": map[string]any{"type": "object", "additionalProperties": true}}, "name", "config")},
	}
}
