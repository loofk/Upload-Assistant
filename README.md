...
- `job_final_report` 是 job status、summary 和 list 的统一首读出口，会把当前 job 压成 `verdict=poll` / `needs_resume_preview` / `needs_resume_execute` / `duplicate_stopped` / `blocked` / `ready_to_report`，并固定暴露 `recommended_call`、进度阶段、重复检查、manual/live/closure 摘要、`complete_when`、`stop_when` 和 blockers；AI 应先读它，再下钻 `job_control_summary`、`live_user_report`、`manual_retorrent_final_report` 或 `recovery_handoff`。
