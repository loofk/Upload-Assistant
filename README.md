...
- `summary-check --json` 会暴露 `flow_diagnostics`、`credential_requirements`、`source_mode`、`target_mode` 和 `automation_reason`；`--print-next-command` 可只输出下一条安全续跑命令，`--print-next-argv` 可输出对应 argv JSON；`--print-shell` 可输出 `PTCLI_*` shell 变量（含 `PTCLI_SOURCE_MODE` / `PTCLI_TARGET_MODE` / `PTCLI_AUTOMATION_REASON` / `PTCLI_FLOW_READY` / `PTCLI_CREDENTIAL_REQUIREMENTS`）；`--run-next-command` 可直接执行下一条受限的 `ptcli.py` 续跑命令。
